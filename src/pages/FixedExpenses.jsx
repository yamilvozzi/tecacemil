  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'var(--bg-input)', borderRadius: 'var(--radius-sm)', padding: '0.4rem 0.6rem', border: '1px solid var(--accent)' }}>
      <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
        style={{ width: 100, padding: '0.25rem 0.5rem', fontSize: '0.82rem' }} autoFocus />
...
function FixedRow({ fx, isApplied, onApply, onUndo, onEdit, onDelete, monthLabel }) {
  ...
  return (
    <tr>
      ...
      <td style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>{fx.category}</td>
      <td>{pmTag(fx.payment_method)}</td>
      <td style={{ textAlign: 'right', fontWeight: 600, color: 'var(--red)' }}>{fmt(fx.amount)}</td>
      <td style={{ minWidth: 190 }}>
        {isApplied ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', color: 'var(--green)', fontSize: '0.78rem', fontWeight: 600 }}>
              <IconCheck /> Aplicado en {monthLabel}
            </div>
            <button className="btn btn-ghost"
              style={{ fontSize: '0.75rem', padding: '0.2rem 0.45rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}
              onClick={handleUndo} disabled={undoLoading} title="Deshacer">
              <IconUndo /> {undoLoading ? '…' : 'Deshacer'}
            </button>
          </div>
        ) : showApply ? (
          ...
          <button className="btn btn-soft"
            style={{ fontSize: '0.78rem', padding: '0.28rem 0.7rem', display: 'flex', alignItems: 'center', gap: '0.35rem' }}
      ...
      <div className="card" style={{ padding: 0 }}>
        <div className="table-wrapper">
          <table>
            <thead>
              <tr>
                <th>Nombre</th><th>Categoría</th><th>Medio</th>
                <th style={{ textAlign: 'right' }}>Monto</th>
                <th>Estado en {monthLabel}</th><th></th>
              </tr>
            </thead>
            <tbody>
              {items.map(fx => (
